        ...
        stage('QA: Build and Lint - ' + config.backendMode) {
            withDocker(config) {
                withCredentials([usernamePassword(credentialsId: "LOCAL_NEXUS", usernameVariable: 'LOCAL_NEXUS_USERNAME', passwordVariable: 'LOCAL_NEXUS_PASSWORD')]) {
                    sh """
                    # Build
                    ${getGradleCommand(config)} clean build -x :sparkling-water-py:build -x check scalaStyle -PlocalNexusUsername=$LOCAL_NEXUS_USERNAME -PlocalNexusPassword=$LOCAL_NEXUS_PASSWORD
                    """
